...
(function(venn) {
        ...
        function chart(selection) {
            selection.each(function(data) {
                ...
                var enterText = enter.append("text")
                    ...
                    .text(function (d) { console.log(d); return label(d); } )
                ...
                var updateText = update.select("text")
                    ...
                    .each("end", venn.wrapText(circles, label2))
                    .attr("x", function(d) {
    ...
    function computeTextCentre(interior, exterior) {
        ...
        for (i = 0; i < interior.length; ++i) {
            var c = interior[i], d = 2 * c.radius/3;
            points.push({x: c.x, y: c.y});
            points.push({x: c.x, y: c.y - d});
            points.push({x: c.x, y: c.y + d});
            points.push({x: c.x - d, y: c.y});
            points.push({x: c.x + d, y: c.y});
        }
        ...
        var solution = venn.fmin(
                    ...
                    [initial.x, initial.y],
                    {maxIterations:5000, minErrorDelta:1e-10}).solution;
